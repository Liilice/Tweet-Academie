...
<head>
    ...
    <script src="./js/specific_hashtag.js"></script>
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.7.1/jquery.min.js"></script>

    <title>Hashtags #<?=$_GET["hashtag"]?> </title>
    <script defer>
        let hashtag = new URLSearchParams(window.location.search).get("hashtag");
        console.log(hashtag);
        $.ajax({
            url: `/back/all_specific_hashtag.php?hashtag=${hashtag}`,
...
<body class="bg-black">
    ...
    <main class="ml-80">
        <div class="flex flex-col w-3/5">
            <form action="./hashtags_page.php" method="POST" class="flex justify-between items-center text-white text-2xl py-4" id="submit">
                <input type="search" name="" id="" class="w-full rounded-full bg-gray-800 text-white mr-5" value="#<?=$_GET["hashtag"]?>">
                <i class="fa-solid fa-magnifying-glass text-sm"></i>
            </form>
            <div id="container" class="flex flex-col " style="color: white;">
            </div>
        </div>
